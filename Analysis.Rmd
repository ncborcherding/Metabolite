...
for (x in seq_along(metabolites)){
  tmp.met <- na.omit(data.frame(metabolites = Liver[,metabolites[x]], Condition = Liver$Condition))
        fit2 <- ss(tmp.met[,1], tmp.met[,2], all.knots = TRUE)
        test <- Liver[,c("Condition", metabolites[x])] %>%
          group_by(Condition) %>%
          na.omit() %>%
          summarize_all(funs(mean))
        ...
        ratio <- fit2$ratio
        lambda <- fit2$lambda
        output <- c(T50 = out, M50 = t50, rate = rate, range = range[,1], FC = FC[,1], GCV = GCV, degF = deg, spar = spar, pen.crit = pen, ratio = ratio, lambda = lambda)
...
ggplot(mat, aes(x=-scale(T50), y=FC)) + 
    ...
    theme_classic() + 
    labs(x = "t50 (Z-score)", y="Metabolite Fold-change")  
ggsave("output/urchin/overall_Urchin_noXcontrol_noYcontrol.pdf", height=5, width=5) 
...
library(htmlwidgets)
saveWidget(p, "p1.html", selfcontained = T)
```
