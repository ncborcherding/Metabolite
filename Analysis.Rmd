...
for (x in seq_along(metabolites)){
        ...
        T0vOther <- tukey.results$`as.factor(tmp.met[, 2])`[1:4,4]
        names(T0vOther) <- paste0("TukeyP.", names(T0vOther))
...
for(i in seq_len(nrow(mat))) {
  time.placeholder <- min(which(mat[i,12:15] < 0.05))
  if (is.null(time.placeholder) | is.na(time.placeholder)) {
    next()
  } else {
